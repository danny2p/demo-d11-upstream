...
class FederatedIndexer {
  ...
  protected function createSolrDocument(array $item, $site_id, $site_url) {
    ...
    $doc->setField('ss_search_api_id', $solr_id);
    $doc->setField('ss_search_api_datasource', 'federated:' . $entity_type);
    $doc->setField('ss_search_api_language', $item['language'] ?? 'en');

    // Add content fields
    $doc->setField('tm_title', $item['title']);

    if (!empty($item['body'])) {
      $doc->setField('tm_body', $item['body']);
    }

    if (!empty($item['summary'])) {
      $doc->setField('tm_summary', $item['summary']);
    }
  ...
  protected function getSolrClient() {
    // Get Solr connection from Pantheon environment
    $host = getenv('PANTHEON_INDEX_HOST');
    $port = getenv('PANTHEON_INDEX_PORT');
    $path = getenv('PANTHEON_INDEX_PATH');
    $core = getenv('PANTHEON_INDEX_CORE');

    if (empty($host) || empty($port) || empty($path) || empty($core)) {
      throw new \Exception('Solr connection not configured');
    }
    ...
    $adapter = new Curl();
    return new Client($adapter, NULL, $config);
  }
  ...
  public function getStatus($site_id = NULL) {
    try {
      ...
      $result = $solr_client->select($query);
